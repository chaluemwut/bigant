...
def main():
    ...
    for iteration in range(iterations):
        ...
        for ant in range(ant_count):
            SumPath =pathAnt[ant][0]+pathAnt[ant][1]+pathAnt[ant][2]
            SumParameter = cityA[pathAnt[ant][0]]+cityB[pathAnt[ant][1]]+cityC[pathAnt[ant][2]]
            lda = gensim.models.ldamodel.LdaModel(corpus=mm, id2word=dictx, num_topics=cityC[pathAnt[ant][2]],alpha=cityA[pathAnt[ant][0]],
update_every=1, chunksize=2000, passes=1,eta=cityB[pathAnt[ant][1]], decay=0.5, offset=1.0, eval_every=10, iterations=1000,
gamma_threshold=0.001, minimum_probability=0.01)
            ...
            #print "path::ant",ant,"::",pathAnt[ant] , "Parameter1::" , cityA[pathAnt[ant][0]]," Parameter2::" ,cityB[pathAnt[ant][1]]," Parameter3::" ,cityC[pathAnt[ant][2]], 
            #print "Perplexity: %s" % perplex ,"   Per-word Perplexity: %s" % per_word_perplex
            print "path:ant",ant,":",pathAnt[ant] , "Param1:" , cityA[pathAnt[ant][0]],"Param2:" ,cityB[pathAnt[ant][1]],"Param3:" ,cityC[pathAnt[ant][2]] 
            print "        Perplexity:%s" % perplex ,"   Per-word Perplexity:%s" % per_word_perplex
            
            #for i in range(0, lda.num_topics):
             #   print 'Topic:: '+str(i) +' '+lda.print_topic(i)
   #         if SumParameter < globalBest:
			#globalBest = SumParameter
